...
Current status: **v1.2.0-rc.3 source candidate targets OMP 18.3.0; targeted two-realm live acceptance has run, but publication is not approved.** Historical v1.2.0-rc.1 approval on OMP 18.2.7 remains recorded below; stable publication remains pending RC observation and final review.
...
Final checks after the isolated run: `npm test` and `npm run test:fast` each passed 20/20 scripts; typecheck, both OpenSpec strict validations and the 13-file pack dry-run passed. A temporary real 18.3.0 AuthStorage probe verified namespaced credential CRUD, session-specific `active`, resolver and Usage APIs. Existing authenticated Intl/CN Hy3 Chat, CN Read and both management commands passed. A later, separately isolated official CLI profile completed fresh OAuth and Hy3 Chat for both realms, cross-process restart, forced expiry followed by real refresh and Chat for both realms, simultaneous Intl/CN Chat, and CN then Intl scoped logout. After CN logout, CN had zero credential rows and zero selectable models, while Intl Chat still succeeded; a separate post-logout CN Chat was not sent. A real CN Task returned `42`; observer hook metadata confirmed five `session_start` and nine `before_provider_request` events on `workbuddy-cn/hy3`, not atomic Bearer/Header identity. The plugin diagnosed `ok` and was uninstalled; the isolated profile and temporary observer were removed. One earlier concurrent probe in the default profile exceeded 110 seconds without stage events; later successful instrumented probes do not identify its cause or exclude recurrence. Controlled host-transport tests cover 401 retry; the CN disabled-Usage test covers zero calls. Unchanged M3 parser/vision/tool capability evidence remains historical, not a new 18.3.0 run. The installed 18.3.0 HTTP inspector still does not redact `X-User-Id` by name; no new 400/413 dump was generated. Publication and tag are not approved: investigate the uninstrumented timeout or explicitly accept the intermittent risk before reconsideration. Exact redacted results and boundaries: [`openspec/changes/upgrade-omp-18-3-auth-storage/evidence.md`](../../openspec/changes/upgrade-omp-18-3-auth-storage/evidence.md).
