...
Current status: **the `v1.2.0-rc.3` GitHub tag targets OMP 18.3.0 and contains the approved, targeted two-realm RC release evidence.** The historical concurrency timeout remains an explicitly accepted RC-only residual risk, not a repaired or diagnosed failure. Historical v1.2.0-rc.1 approval on OMP 18.2.7 remains recorded below; stable publication remains pending RC observation and final review.
...
OMP 18.3.0 removed the flat `AuthStorage.listOAuthAccounts`, `remove`, `invalidateUsageCache`, `fetchUsageReports`, and `resolver` methods. The 18.3.0 runtime exposes `oauth.accounts`, `credentials.remove`, `usage.invalidate`/`reports`, and `keys.resolver` instead. WorkBuddy migrated the Provider and UI paths; test credentials use the new namespace. No WorkBuddy OAuth, Gateway, or Billing protocol was changed. Provider single-account checks remain session-independent; UI account observations remain session-aware. This host API cutover alone does not establish live OAuth or atomic request-identity binding; separately executed live cases appear below.

After the release-decision documentation update, `npm test` and `npm run test:fast` each passed 20/20 scripts; typecheck, both OpenSpec strict validations and the 13-file pack dry-run passed. A fresh no-credential official profile also passed local install, plugin doctor (`ok`; fresh-profile `package_manifest` warning), uninstall and empty plugin list; it was removed. A temporary real 18.3.0 AuthStorage probe verified namespaced credential CRUD, session-specific `active`, resolver and Usage APIs. Existing authenticated Intl/CN Hy3 Chat, CN Read and both management commands passed. An official CLI isolated profile completed fresh OAuth and Hy3 Chat for both realms, cross-process restart, forced expiry followed by real refresh and Chat for both realms, simultaneous Intl/CN Chat, and CN then Intl scoped logout. After CN logout, CN had zero credential rows and zero selectable models, while Intl Chat still succeeded; a separate post-logout CN Chat was not sent. A real CN Task returned `42`; observer metadata confirmed five `session_start` and nine `before_provider_request` events on `workbuddy-cn/hy3`, not atomic Bearer/Header identity. The profile and observer were removed.

A new isolated profile with one fresh login per realm ran 10 consecutive, instrumented simultaneous Intl/CN Hy3 Chat pairs. All 20 processes exited 0, returned their per-run markers, recorded matching session/request and agent/message/end stages, and completed within the predeclared 30-second threshold; the longest Intl/CN runs took 3419/2961 ms. No error events, malformed event rows, or stderr were observed. Official plugin install and doctor (`ok`) preceded the run; uninstall left an empty plugin list, then only this profile and temporary stage files were removed. Production implementation `a2c2b71`, first isolated live baseline `b5eb766`, repeated-concurrency baseline `0f620f3`, and risk-approval documentation commit `0b066ea` are distinct from the immutable final release commit identified by `git rev-parse 'v1.2.0-rc.3^{commit}'`; no intervening runtime or package-declaration change was made.

One earlier uninstrumented concurrent probe in the default profile exceeded 110 seconds. Later success does not identify its cause, prove a fix, or bound the failure rate; the RC gate accepts that unresolved incident as a documented residual risk, not as a host-specific diagnosis. Controlled host-transport tests cover 401 retry; the CN disabled-Usage test covers zero calls. Unchanged M3 parser/vision/tool capability evidence remains historical, not a new 18.3.0 run. The installed 18.3.0 HTTP inspector still does not redact `X-User-Id` by name; no new 400/413 dump was generated. Redacted outcomes and release decision: [`openspec/changes/upgrade-omp-18-3-auth-storage/evidence.md`](../../openspec/changes/upgrade-omp-18-3-auth-storage/evidence.md).
