...
Local checks: `npm test` and `npm run test:fast` each passed 20/20 scripts; typecheck, OpenSpec strict and pack dry-run passed. A temporary real 18.3.0 AuthStorage probe verified namespaced credential CRUD, session-specific `active`, resolver and Usage APIs. The official host installed, diagnosed and uninstalled the candidate in an isolated profile. Existing authenticated Intl/CN Hy3 headless Chat passed, CN Read returned the exact package version, and `/workbuddy` showed credits available while `/workbuddy-cn` showed unavailable. An Intl Task invocation occurred, but its child model/identity was not independently confirmed. One earlier concurrent probe exceeded 110 seconds without stage events; a later instrumented simultaneous run returned both realm markers, with first `message_update` at 3.1/1.5 seconds and process exit at 12.8/10.8 seconds. The original timeout cause is unknown. Auth-affected fresh OAuth, restart, forced refresh, scoped logout and confirmed Task identity remain unverified. Controlled 18.3.0 host-transport tests cover 401 retry; the CN disabled-Usage test covers zero calls. Unchanged M3 parser/vision/tool capability evidence remains historical, not a new 18.3.0 run. The installed 18.3.0 HTTP inspector still does not redact `X-User-Id` by name; no new 400/413 dump was generated. Detailed results and pending gates: [`openspec/changes/upgrade-omp-18-3-auth-storage/evidence.md`](../../openspec/changes/upgrade-omp-18-3-auth-storage/evidence.md).
