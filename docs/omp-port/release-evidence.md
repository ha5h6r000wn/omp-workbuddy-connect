...
Current status: **v1.2.0-rc.3 on OMP 18.3.0 has passed the targeted RC release gate, with the unresolved historical concurrency timeout explicitly accepted as an RC-only residual risk; no tag has been created or pushed.** Historical v1.2.0-rc.1 approval on OMP 18.2.7 remains recorded below; stable publication remains pending RC observation and final review.
...
A new isolated profile with one fresh login per realm ran 10 consecutive, instrumented simultaneous Intl/CN Hy3 Chat pairs. All 20 processes exited 0, returned their per-run markers, recorded matching session/request and agent/message/end stages, and completed within the predeclared 30-second threshold; the longest Intl/CN runs took 3419/2961 ms. No error events, malformed event rows, or stderr were observed. Official plugin install and doctor (`ok`) preceded the run; uninstall left an empty plugin list, then only this profile and temporary stage files were removed. Production implementation `a2c2b71`, first isolated live baseline `b5eb766`, and repeated-concurrency baseline `0f620f3` have unchanged runtime/package declarations; the final documentation/tag commit is still pending.
